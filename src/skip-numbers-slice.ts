...
export const createSkipNumbersSlice: SliceStateCreator<SkipNumbersSlice> = (set) => ({
  skipNumbers: [],
  addSkipNumber: (skipNumber: number) => set((state) => {
    state.skipNumbers.push(skipNumber)
  }),
  updateSkipNumber: (index: number, skipNumber: number) => set((state) => {
    state.skipNumbers[index] = skipNumber
  }),
  removeSkipNumber: (index: number) => set((state) => {
